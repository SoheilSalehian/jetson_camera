...
private:
    static std::string gstreamer_pipeline (int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
        return "nvarguscamerasrc sensor_id=0 ! video/x-raw(memory:NVMM), width=(int)" + std::to_string(capture_width) + ", height=(int)" + 
               std::to_string(capture_height) + ", format=(string)NV12, framerate=(fraction)" + std::to_string(framerate) + "/1 ! " +
               "nvvidconv flip-method=" + std::to_string(flip_method) + " ! video/x-raw, width=(int)" + std::to_string(display_width) + ", height=(int)" + 
    ...
    void onInit() override
    {
        ...
        is_running_ = true;
        cap_thread_ = std::make_shared<std::thread>(std::bind(&JetsonCameraNodelet::captureFunc, this));
    }
    ...
    void captureFunc()
    {
        ...
        if (!cap.isOpened()) {
            NODELET_ERROR("Failed to open camera with pipeline: %s", pipeline.c_str());
            return;
        }
        NODELET_INFO("Camera opened successfully");
        ...
        while (is_running_.load())
        {
            if (cap.read(img.image))
            {
                ...
                if (frame_count % 30 == 0) {  // Log every 30 frames
                    NODELET_INFO("Published frame %d", frame_count);
                }
            }
            else
            {
                NODELET_WARN("Failed to capture frame");
            }
        }
    }
};
